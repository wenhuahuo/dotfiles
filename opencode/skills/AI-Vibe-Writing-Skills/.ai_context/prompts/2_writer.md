...
1. **Project Context**: 只读取当前项目的 `.ai_context/`；skill 自带 `.ai_context/` 仅作为模板参考。
2. **Style Profile**: 请严格遵循 `style_profile.md` 中的语调、句式和用词习惯。
3. **Error Log**: 请严格遵守 `error_log.md` 中的“禁忌清单”。在生成任何文本前，必须自查是否触犯了里面的规则。
4. **Long-Term Memory**: 读取 `.ai_context/memory/hard_memory.json` 与 `.ai_context/memory/soft_memory.json`，按领域对齐硬性与柔性记忆。
5. **Reference Library**: 读取 `.ai_context/custom_specs.md` 中的 Reference Learning Settings 与 Evidence Requirements，并加载 `reference_library.json` 的可用证据。
...
9. **Revision Trigger**: 如果来自检阅 Agent 的报告包含必须修改项，直接输出修订后的正文，不得只解释建议。
